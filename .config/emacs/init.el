...
;;       in Emacs and init.el will be generated automatically!
  ...
  (defvar gw/default-font-size 180)
  (defvar gw/default-variable-font-size 180)

  ;; Make frame transparency overridable
  (defvar gw/frame-transparency '(90 . 90))

  ;; The default is 800 kilobytes.  Measured in bytes.
  (setq gc-cons-threshold (* 50 1000 1000))

  (defun gw/display-startup-time ()
  ...
  (add-hook 'emacs-startup-hook #'gw/display-startup-time)
...
;; Disable the menu bar for real this time
...
;; Change the title bar format to my custom message
  ...
  (set-frame-parameter (selected-frame) 'alpha gw/frame-transparency)
  (add-to-list 'default-frame-alist `(alpha . ,gw/frame-transparency))
...
(set-face-attribute 'default nil :font "Jetbrains Mono" :height gw/default-font-size)

;; Set the fixed pitch face
(set-face-attribute 'fixed-pitch nil :font "Jetbrains Mono" :height gw/default-font-size)

;; Set the variable pitch face
(set-face-attribute 'variable-pitch nil :font "Ubuntu" :height gw/default-variable-font-size :weight 'regular)
  ...
  (use-package general
    :after evil
    :config
    (general-create-definer gw/leader-keys
...
(global-set-key (kbd  "SPC g m") #'magit)
...
;; )
  ...
  (use-package org
    ...
    :hook (org-mode . gw/org-mode-setup)
    :config
    ;; (setq org-ellipsis " ▾")
    (setq org-ellipsis " ↴")
    ...
    (setq org-agenda-files '("~/Documents/agenda.org" "~/Documents/To-Research.org")
  ...
  (defun gw/org-mode-visual-fill ()
    (setq visual-fill-column-width 100
          visual-fill-column-center-text t)
    (visual-fill-column-mode 1))

  (use-package visual-fill-column
    :hook (org-mode . gw/org-mode-visual-fill))
  ...
  (defun gw/org-babel-tangle-config ()
  ...
  (add-hook 'org-mode-hook (lambda () (add-hook 'after-save-hook #'gw/org-babel-tangle-config)))

  (defun gw/lsp-mode-setup ()
    (setq lsp-headerline-breadcrumb-segments '(path-up-to-project file symbols))
    (lsp-headerline-breadcrumb-mode))

  (use-package lsp-mode
    :commands (lsp lsp-deferred)
    :hook (lsp-mode . gw/lsp-mode-setup)
    :init
...
(use-package rainbow-delimiters
  ...
  (use-package eshell
    :hook (eshell-first-time-mode . gw/configure-eshell)
    :config
